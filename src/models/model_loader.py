...
class ModelLoader:
    ...
    @staticmethod
    def _resolve_hf_snapshot_path(model_id: str) -> str:
        """Resolve a local Hugging Face snapshot path when available."""
        ...
        if snapshot_hash:
            candidate = os.path.join(repo_dir, 'snapshots', snapshot_hash)
    ...
    def get_clip_source(model_name: str) -> str:
        ...
        return ModelLoader._resolve_hf_snapshot_path(ModelLoader.CLIP_MODELS[model_name])

    @staticmethod
    def get_caption_source(model_name: str) -> str:
        """Resolve caption model source."""
        return ModelLoader._resolve_hf_snapshot_path(ModelLoader.CAPTION_MODELS[model_name])
    ...
    def _load_caption_model(model_name: str, pretrained: bool, device: str) -> nn.Module:
        ...
        model_path = ModelLoader.get_caption_source(model_name)

        if 'blip' in model_name:
            if os.path.isdir(model_path):
                base_model = BlipForConditionalGeneration.from_pretrained(model_path, local_files_only=True)
                ...
                processor = BlipProcessor.from_pretrained(model_path, cache_dir=HF_CACHE_DIR)
        else:
        ...
        return ImageCaptionModel(base_model, processor=processor)

    @staticmethod
    def load_caption_model(model_name: str, device: str = 'cuda') -> nn.Module:
        """Load a caption model for multimodal evaluation only."""
        if model_name not in ModelLoader.CAPTION_MODELS:
            raise ValueError(f"Unsupported caption model: {model_name}")
        device = device if torch.cuda.is_available() else 'cpu'
        return ModelLoader._load_caption_model(model_name, pretrained=True, device=device)
