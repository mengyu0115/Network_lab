...
class CLIPMultimodalEvaluator:
    ...
    def _tensor_batch_to_pil(self, images: torch.Tensor):
        pil_images = []
    ...
    def evaluate(
        ...
        orig_pil_images = self._tensor_batch_to_pil(original_image)
        adv_pil_images = self._tensor_batch_to_pil(adversarial_image)

        with torch.no_grad():
            orig_inputs = self.processor(images=orig_pil_images, return_tensors="pt").to(self.device)
            adv_inputs = self.processor(images=adv_pil_images, return_tensors="pt").to(self.device)
            orig_img_feat = self._normalize(self._as_feature_tensor(self.clip_model.get_image_features(**orig_inputs)))
            adv_img_feat = self._normalize(self._as_feature_tensor(self.clip_model.get_image_features(**adv_inputs)))
