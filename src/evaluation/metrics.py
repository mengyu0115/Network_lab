...
class CLIPEvaluator:
    ...
    def _tensor_batch_to_pil(self, images: torch.Tensor) -> List:
        """Convert a BCHW tensor batch into a PIL list for processor-driven multimodal scoring."""
        pil_images = []
    ...
    def evaluate(self, original_images: torch.Tensor,
        ...
        if text_prompts:
            ...
            original_images = original_images.to(self.device)
            adversarial_images = adversarial_images.to(self.device)
            orig_pil_images = self._tensor_batch_to_pil(original_images)
            adv_pil_images = self._tensor_batch_to_pil(adversarial_images)

            with torch.no_grad():
                orig_inputs = self.model.processor(
                    text=text_prompts, images=orig_pil_images, return_tensors="pt", padding=True
                ).to(self.device)

                adv_inputs = self.model.processor(
                    text=text_prompts, images=adv_pil_images, return_tensors="pt", padding=True
                ).to(self.device)
