...
class CLIPMultimodalAttack:
    ...
    def generate(
        ...
        for _ in range(self.num_steps):
            ...
            if need_img_grad:
                ...
                if grad_img is not None:
                    adv_image = adv_image - self.image_alpha * grad_img.sign()
                    img_pert = torch.clamp(adv_image - image, -self.image_epsilon, self.image_epsilon)
                    adv_image = torch.clamp(image + img_pert, 0.0, 1.0).detach()
        ...
        return adv_image.detach(), adv_text_feat.detach(), info
